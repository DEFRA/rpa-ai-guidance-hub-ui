...
import * as stagedDocumentApi from '../infra/guidance-api/staged-document.js'
 ...
 *
 * @param {string} fileId
 * @returns {Promise<StagedDocumentStatusModel|null>} Shaped staged document
 *   status, or null if no staged document has been claimed yet for this file ID
 * @throws {GuidanceApiError} - If the infra layer reports a non-404 failure
 */
async function getStagedDocumentById (fileId) {
  const res = await stagedDocumentApi.getStagedDocument(fileId)
